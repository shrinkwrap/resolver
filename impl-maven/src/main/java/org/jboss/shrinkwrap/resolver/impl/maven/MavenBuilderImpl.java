...
public class MavenBuilderImpl implements MavenDependencyResolverInternal
{
    ...
    /**
     * Configures Maven from a settings.xml file. It first tries to find the file locally, and then it looks for it on the
     * classpath.
     * 
     ...
     * central repository by default. It first tries to find the file locally, and then it looks for it on the classpath.
     * 
    ...
    public <ARCHIVEVIEW extends Assignable> Collection<ARCHIVEVIEW> resolveAs(final Class<ARCHIVEVIEW> archiveView,
        ...
        {
            final ARCHIVEVIEW archive = ShrinkWrap.create(ZipImporter.class, file.getName()).importFrom(this.convert(file))
                    .as(archiveView);
