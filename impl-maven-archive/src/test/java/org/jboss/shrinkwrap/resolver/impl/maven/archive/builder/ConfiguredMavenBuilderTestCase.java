...
import static org.jboss.shrinkwrap.resolver.impl.maven.archive.builder.ArchiveContentMatchers.size;
 ...
 *
 */
public class ConfiguredMavenBuilderTestCase {
    ...
    public void importJar() {
        // When
        final Archive<?> archive = ShrinkWrap.create(MavenBuilder.class).configureFromFile(SETTINGS_FILE)
    ...
    public void importJarOffline() {
        ...
        ShrinkWrap.create(MavenBuilder.class).configureFromFile(SETTINGS_FILE).offline()
                .loadPomFromFile("src/it/jar-sample/pom.xml").importBuildOutput().as(WebArchive.class);
