...
import org.jboss.shrinkwrap.resolver.api.maven.filter.AcceptDependenciesFilter;
 ...
 *
 */
public class WarArchiveMavenAssemblerTestCase {
    ...
    public void importWarWithName() {
        ...
        // When
        final WebArchive archive = ShrinkWrap.create(ArchiveMavenAssembler.class)
            .usingPom("src/it/war-sample/pom.xml")
            .withBuildOutput()
            .withDependencies(ScopeType.COMPILE)
            .as(WebArchive.class, name);
        ...
        assertThat(archive.getContent(), not(contains("file.packagingToExclude")));
        assertThat(archive.getContent(), not(contains("file.warSourceToExclude")));
        System.out.println(archive.getContent().size());
    ...
    private WebArchive doImport(String pomFile) {
        ...
        WebArchive archive = ShrinkWrap.create(ArchiveMavenAssembler.class).usingPom(pomFile).withBuildOutput()
            .withDependencies(new AcceptDependenciesFilter("commons-codec:commons-codec:?")).as(WebArchive.class);
