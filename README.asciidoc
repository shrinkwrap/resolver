...
Importer is configured to execute by default +build --exclude-task test -PbuildDir=build-swr+, what means it skips tests and puts the build results in
separate +build-swr+ directory. It's possible to alter this behaviour just by using appropriate API methods like e.g. +forTasks+ or +withArguments+.
